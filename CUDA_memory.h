...
template<class T>
std::string print_buffer(const smart_cpu_buffer<T> arr)
{
    std::string result = "{ ";
    for (int i = 0; i < arr.dedicated_len - 1; i++)
        result += std::to_string(arr.cpu_buffer_ptr[i]) + ", ";
    return result + (arr.dedicated_len > 0 ? std::to_string(arr.cpu_buffer_ptr[arr.dedicated_len - 1]) : "") + " }";
}

template<class T>
std::string print_buffer(const smart_gpu_cpu_buffer<T> arr)
{
    std::string result = "{ ";
    for (int i = 0; i < arr.dedicated_len - 1; i++)
        result += std::to_string(arr.cpu_buffer_ptr[i]) + ", ";
